...
- Hiring Pipeline uses fixed interview stage templates selected per job post. Candidate applications inherit the selected template stages; per-candidate custom stage flows are future scope and should only appear in tooltips or documentation.
- Workflows follows the official Stitch routing-rules reference where possible: header actions, three tabs, fallback banner, compact filter toolbar, routing rules table, and resolver config card. Keep only three active Admin tabs: Global Policies, Routing Rules, and Transition Triggers. Do not show a `Later Phase` or escalation tab. Routing to Hiring Manager after final interview is active MVP behavior, not a later-phase placeholder. Do not show Email/SignalR channel controls on Workflows; backend notification handlers send those automatically after handoff events.
